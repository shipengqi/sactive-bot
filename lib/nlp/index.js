...
class NLP {
  ...
  misspelling(command) {
    ...
    this.logger.debug(`${logPrefix} '${command}' is help, similarity: ${helpSimilarity}`);
    ...
    if (integrations.length > 0) {
      ...
      for (let integration of integrations) {
        ...
        fullCommands = fullCommands.concat(this._getSimilarStrings(fakeCommand, subCommands.keys(), `@${botName} ${integration.command}`));
      }
      result.isCommand = true;
    ...
    for (let [integration, commandInfo] of commands.entries()) {
      fullCommands = fullCommands.concat(this._getSimilarStrings(fakeCommand, commandInfo.subs.keys(), `@${botName} ${integration}`));
    }
    result.isCommand = true;
    result.similarList = fullCommands;
    return result;
  }
  // if integration is not empty, then splices botName, integration and command, parameters together, try to get the full commands
  _getSimilarStrings(string, subCommands, prefix) {
    let similarCommands = [];
    for (let command of subCommands) {
      let supposedFullCommand = prefix ? `${prefix} ${command}` : command;
      let originString = prefix ? `${prefix} ${string}` : string;
