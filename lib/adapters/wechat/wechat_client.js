...
class WechatClient extends EventEmitter {
  constructor(adapter, options) {
    ...
    // todo add cookie for wechat
    // this.cookie = options.cookie;

    this.acceptFriend = options.acceptFriend;
    if (this.acceptFriend === 'no' || this.acceptFriend === 'n') {
      this.acceptFriend = false;
    }
    this.ignoreGroupMessage = options.ignoreGroupMessage;
  ...
  async init(options) {
    ...
    this.pass_ticket = options.pass_ticket;
        ...
        if (!this.initialized) {
          ...
          if (_initRsp.BaseResponse.Ret !== 0) {
            ...
            this.robot.master = _.find(contactRsp, u => {
              let master = this.options.SBOT_WX_MASTER;
  ...
  async _syncCheck() {
    ...
    try {
      ...
      // }
      this.robot.logger.warn('Sync check failed.');
      this.robot.logger.debug(e);
      // Try to sync message when catch any error
      return process.nextTick(this._syncCheck.bind(this));
    }
  }
