...
class WeChatAuthServer {
  ...
  getQRCode() {
    ...
    app.init();
    app.listen(Number(this.serverPort));
    this.logger.info(`${WECHAT_ADAPTER_PREFIX} Access ****http://localhost:8082/login****, and scan the QR code to login.`);
  }
  ...
  async run() {
    ...
    this.getQRCode();
    while (true) {
      this.logger.info(`${WECHAT_ADAPTER_PREFIX} Confirm the login on your phone.`);
