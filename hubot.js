...
const fs = require('fs');
const SlackAuth = require('./lib/adapters/slack').Auth;
const hubotServer = require('./lib/command_receiver/hubot_server');
...
const {
  ...
  ADAPTER_MAPPINGS,
  DELETE_LOG_TIME,
  ACTION_SESSION_DEFAULT_TTL,
  CLEAR_ACTION_SESSION_TIME
} = require('./lib/const');
const {
  EnterpriseRobot,
  createRobotAdapter
...
function loadBot() {
  ...
  hubotServer.run(unifiedConfig, function(unifiedApp, unifiedServer, keys) {
    robot.unifiedApp = unifiedApp;
    robot.unifiedServer = unifiedServer;
...
function loadSlackBot() {
  if (slackApiToken && slackBotToken) {
    ...
    cache.getTokens(botName).then(function(tokens) {
      ...
      checkPortStatus(slackServerPort, {timeout: DELAY_TIME})
        .then(function(status) {
          ...
          // Run auth flow
          return server.run();}).then(function(tokens) {
        ...
        cache.saveTokens(botName, tokens);}).then(function() {
        // Load the bot
        loadBot();
...
// delete log every one week(at every Tue)
clearLog(DELETE_LOG_TIME);
// clear expired session for aciton
setInterval(function () {
  for (let [key, value] of ACTION_SESSION_MAP.entries()) {
    if(value.accessTime) {
      let now = new Date();
      let nowTime = now.getTime() / 1000;
      if((nowTime - value.accessTime) > ACTION_SESSION_DEFAULT_TTL) {
        logger.debug(`Delete invalid session id ${key}`);
        ACTION_SESSION_MAP.delete(key);
      }
    }
  }
}, CLEAR_ACTION_SESSION_TIME);
